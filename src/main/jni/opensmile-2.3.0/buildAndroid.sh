...
##flags
export OPTIMISE="-fPIE -pie -fPIC -ffast-math -ftree-vectorize -march=armv7-a -mthumb -mfloat-abi=softfp -mfpu=vfp -mfpu=neon -funsafe-math-optimizations";
export LDFLAGS="-fPIE -pie -fPIC -fexceptions -Wl,-rpath-link=${NDK}/platforms/${platform}/arch-arm/usr/lib -L${NDK}/platforms/${platform}/arch-arm/usr/lib -L${NDK}/sources/cxx-stl/gnu-libstdc++/${STL_VERSION}/libs/armeabi"
export LDFLAGS_CORE="-fPIE -fPIC -pie -fexceptions -L${NDK}/platforms/${platform}/arch-arm/usr/lib -L${NDK}/sources/cxx-stl/gnu-libstdc++/${STL_VERSION}/libs/armeabi"
export SYSROOT=${NDK}/platforms/${platform}/arch-arm
export CXXFLAGS="-fPIC -I${NDK}/platforms/${platform}/arch-arm/usr/include -I${NDK}/sources/cxx-stl/gnu-libstdc++/${STL_VERSION}/include -I${NDK}/sources/cxx-stl/gnu-libstdc++/${STL_VERSION}/libs/armeabi/include --sysroot=${SYSROOT} -g -O3 $OPTIMISE"
DEFINES="-DOPENSMILE_BUILD -DBUILD_LIBSVM -DBUILD_SVMSMO -DBUILD_RNN"

export CPPFLAGS="--sysroot=${SYSROOT} -fPIC -D__ANDROID__ ${DEFINES} -DHAVE_OPENSLES $STATIC_DEFINE"
export CFLAGS="--sysroot=${SYSROOT} -fPIC -nostdlib -g -O3 ${OPTIMISE}"
...
./configure --enable-static --with-pic --enable-shared=no --host=arm-linux-androideabi --target=arm-linux-androideabi LIBS="$LIBS" --prefix="$Pwdd/inst/android";
